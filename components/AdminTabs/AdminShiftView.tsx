...
export default function AdminShiftView({ roster, headers, onUpdateShift }: Props) {
  ...
  const handleEmployeeClick = (emp: any) => {
    ...
    setEditingDateIndex(emp.dateIndex);
    setCurrentShift(emp.shift === 'N/A' ? '' : emp.shift);
    setEditModalOpen(true);
  };

  const handleSaveShift = (newShift: string) => {
    if (editingEmployee && editingDateIndex >= 0) {
      onUpdateShift(editingEmployee.id, editingDateIndex, newShift);
    }
